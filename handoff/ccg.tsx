// apps/mobile/componentes/ccg.tsx — componentes novos do redesenho.
// Requer: expo-linear-gradient, react-native-svg, @expo-google-fonts/manrope,
...
export function BarraCapsula({ state, descriptors, navigation }: any) {
  return (
    <View style={estilos.barraFora}>
      <View style={estilos.capsula}>
        ...
        {state.routes.map((rota: any, i: number) => {
          ...
          return (
            ...
            >
              {options.tabBarIcon?.({ color: cor, focused: ativo })}
...
export function PainelAcao({ children }: { children: React.ReactNode }) {
  const c = useTema()
  return (
...
export function FichaProfessor({ nome, area, iniciais, foto }: any) {
  const c = useTema()
  return (
    <Pressable style={[estilos.ficha, { backgroundColor: c.cartao, borderColor: c.linha }]}>
      {foto ? (
        <Image source={foto} style={estilos.retrato} />
      ) : (
